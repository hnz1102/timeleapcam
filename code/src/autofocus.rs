use log::info;
use esp_camera_rs::CameraSensor;
use std::{thread, time::Duration};

#[allow(dead_code)]
const OV5640_RESET : u16 = 0x3000;
const OV5640_CHIPID_HIGH : u16 = 0x300a;
const OV5640_CHIPID_LOW : u16 = 0x300b;
const OV5640_CMD_MAIN : u16 = 0x3022;
const OV5640_CMD_ACK : u16 = 0x3023;
const OV5640_CMD_PARA0 : u16 = 0x3024;
const OV5640_CMD_PARA1 : u16 = 0x3025;
const OV5640_CMD_PARA2 : u16 = 0x3026;
const OV5640_CMD_PARA3 : u16 = 0x3027;
const OV5640_CMD_PARA4 : u16 = 0x3028;
const OV5640_CMD_FW_STATUS : u16 = 0x3029;
const AF_TRIG_SINGLE_AUTO_FOCUS : u8 = 0x03;
const AF_GET_FOCUS_RESULT : u8 = 0x07;
#[allow(dead_code)]
const AF_RELEASE_FOCUS : u8 = 0x08;
const AF_RELAUNCH_ZONE_CONFIGURATION : u8 = 0x12;
const AF_LAUNCH_DEFAULT_ZONE : u8 = 0x80;
const FW_STATUS_S_FIRMWARE : u8 = 0x7F;
#[allow(dead_code)]
const FW_STATUS_S_STARTUP : u8 = 0x7E;
const FW_STATUS_S_IDLE : u8 = 0x70;
#[allow(dead_code)]
const FW_STATUS_S_FOCUSED : u8 = 0x10;
#[allow(dead_code)]
const FW_STATUS_S_ZONE_CONFIG : u8 = 0x16;
#[allow(dead_code)]
const FW_STATUS_S_FOCUSING : u8 = 0x00; //0x00 - 0X0F, 0x80 - 0X8F
const ALL_MASK : u8 = 0xFF;
...
impl<'a> AutoFocus<'a> {
    ...
    pub fn init(&mut self) {
        ...
        for i in 0..AUTOFOCUS_CONFIG.len() {
            match self.sensor.set_reg((0x8000 + i) as u16, ALL_MASK, AUTOFOCUS_CONFIG[i]){
                Ok(_) => {},
                Err(e) => { info!("Failed to set register: {}", e); }
            }
        }
        let _ = self.sensor.set_reg(OV5640_CMD_MAIN, ALL_MASK, 0x00);
        let _ = self.sensor.set_reg(OV5640_CMD_ACK, ALL_MASK, 0x00);
        let _ = self.sensor.set_reg(OV5640_CMD_PARA0, ALL_MASK, 0x00);
        let _ = self.sensor.set_reg(OV5640_CMD_PARA1, ALL_MASK, 0x00);
        let _ = self.sensor.set_reg(OV5640_CMD_PARA2, ALL_MASK, 0x00);
        let _ = self.sensor.set_reg(OV5640_CMD_PARA3, ALL_MASK, 0x00);
        let _ = self.sensor.set_reg(OV5640_CMD_PARA4, ALL_MASK, 0x00);
        let _ = self.sensor.set_reg(OV5640_CMD_FW_STATUS, ALL_MASK, FW_STATUS_S_FIRMWARE);
        let _ = self.sensor.set_reg(OV5640_RESET, ALL_MASK, 0x00);

        info!("Waiting for autofocus to start...");
        loop {
            thread::sleep(Duration::from_millis(5));
			let status = self.sensor.get_reg(OV5640_CMD_FW_STATUS, ALL_MASK);
            if status == FW_STATUS_S_IDLE {
                break;
            }
        }
    ...
    pub fn autofocus_zoneconfig(&self) {
        ...
        loop {
            thread::sleep(Duration::from_millis(5));
			let status = self.sensor.get_reg(OV5640_CMD_ACK, ALL_MASK);
			if status == 0x00 {
                break;
            }
        }
        let _ = self.sensor.set_reg(OV5640_CMD_ACK, ALL_MASK, 0x01);
        let _ = self.sensor.set_reg(OV5640_CMD_MAIN, ALL_MASK, AF_LAUNCH_DEFAULT_ZONE);
        loop {
            thread::sleep(Duration::from_millis(5));
			let status = self.sensor.get_reg(OV5640_CMD_ACK, ALL_MASK);
			if status == 0x00 {
	...
	pub fn get_focus_result(&self) -> u8 {
		...
		loop {
			...
			let status = self.sensor.get_reg(OV5640_CMD_ACK, ALL_MASK);
			if status == 0x00 {
				let result = self.sensor.get_reg(OV5640_CMD_PARA0, ALL_MASK);
				let _ = self.sensor.get_reg(OV5640_CMD_PARA1, ALL_MASK);
				let _ = self.sensor.get_reg(OV5640_CMD_PARA2, ALL_MASK);
				let _ = self.sensor.get_reg(OV5640_CMD_PARA3, ALL_MASK);
				let _ = self.sensor.get_reg(OV5640_CMD_PARA4, ALL_MASK);
				if result == 0x00 {
					info!("AutoFocus Focused");
				} else {
					info!("AutoFocus Not Focused");
				}
				return result;	// zero is focused
			}
			if count > 2000 {
				info!("AutoFocus Timeout");
				return 0xFF;
